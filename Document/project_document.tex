...
\If {(route is feasible)} customer stays in the route \Else ~customer gets removed again
\EndIf
\If {(no feasible customer can be added to the route)} algorithm starts again for the next route
\EndIf
\end{algorithmic}
\end{algorithm}
...
\If {(new root is better)} keep the new route \Else ~switch it back
\EndIf
